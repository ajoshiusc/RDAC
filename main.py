...
def re_init_phi(phi, dt):
    ...
    neg_y = area_neg[:, 0]
    neg_x = area_neg[:, 1]
    tmp1 = tf.reduce_max([tf.square(tf.gather_nd(t, area_pos))
                         for t in [ap, bn]], axis=0)
    tmp1 += tf.reduce_max([tf.square(tf.gather_nd(t, area_pos))
                          for t in [cp, dn]], axis=0)
    update1 = tf.sqrt(tf.abs(tmp1)) - 1
    indices1 = tf.stack([pos_y, pos_x], 1)
    tmp2 = tf.reduce_max([tf.square(tf.gather_nd(t, area_neg))
                         for t in [an, bp]], axis=0)
    tmp2 += tf.reduce_max([tf.square(tf.gather_nd(t, area_neg))
                          for t in [cn, dp]], axis=0)
    ...
    dD = tf.scatter_nd(indices_final, update_final, shape=[
...
def get_curvature(phi, x, y):
    ...
    y_plus = tf.minimum(tf.cast(y_plus, dtype="int32"),
                        tf.cast(dim_y - 1, dtype="int32"))
    x_plus = tf.minimum(tf.cast(x_plus, dtype="int32"),
                        tf.cast(dim_x - 1, dtype="int32"))
    y_minus = tf.maximum(y_minus, 0)
    x_minus = tf.maximum(x_minus, 0)
    d_phi_dx = tf.gather_nd(phi, tf.stack(
        [y, x_plus], 1)) - tf.gather_nd(phi, tf.stack([y, x_minus], 1))
    d_phi_dx_2 = tf.square(d_phi_dx)
    d_phi_dy = tf.gather_nd(phi, tf.stack(
        [y_plus, x], 1)) - tf.gather_nd(phi, tf.stack([y_minus, x], 1))
...
def get_intensity(image, masked_phi, filter_patch_size=5):
    u_1 = tf.layers.average_pooling2d(tf.multiply(image, masked_phi), [
                                      filter_patch_size, filter_patch_size], 1, padding='SAME')
    u_2 = tf.layers.average_pooling2d(
        masked_phi, [filter_patch_size, filter_patch_size], 1, padding='SAME')
    u_2_prime = 1 - tf.cast((u_2 > 0), dtype='float32') + \
        tf.cast((u_2 < 0), dtype='float32')
...
def active_contour_layer(elems):
    ...
    def _body(i, phi_level):
        band_index = tf.reduce_all(
            [phi_level <= narrow_band_width, phi_level >= -narrow_band_width], axis=0)
        ...
        def body_intensity(j, mean_intensities_outer, mean_intensities_inner):
            ...
            local_window_x_min = tf.cast(
                tf.floor(xnew - window_radius_x), dtype="int32")
            local_window_x_max = tf.cast(
                tf.floor(xnew + window_radius_x), dtype="int32")
            local_window_y_min = tf.cast(
                tf.floor(ynew - window_radius_y), dtype="int32")
            local_window_y_max = tf.cast(
                tf.floor(ynew + window_radius_y), dtype="int32")
            local_window_x_min = tf.maximum(zero_tensor, local_window_x_min)
            local_window_y_min = tf.maximum(zero_tensor, local_window_y_min)
            local_window_x_max = tf.minimum(
                tf.cast(input_image_size - 1, dtype="int32"), local_window_x_max)
            local_window_y_max = tf.minimum(
                tf.cast(input_image_size - 1, dtype="int32"), local_window_y_max)
            local_image = img[local_window_y_min: local_window_y_max +
                              1, local_window_x_min: local_window_x_max + 1]
            local_phi = phi_level[local_window_y_min: local_window_y_max +
                                  1, local_window_x_min: local_window_x_max + 1]
            ...
            mean_intensity_inner = tf.cast(
                tf.divide(tf.reduce_sum(image_loc_inner), area_inner), dtype='float32')
            mean_intensity_outer = tf.cast(
                tf.divide(tf.reduce_sum(image_loc_outer), area_outer), dtype='float32')
            mean_intensities_inner = tf.concat(
                axis=0, values=[mean_intensities_inner[:j], [mean_intensity_inner]])
            mean_intensities_outer = tf.concat(
                axis=0, values=[mean_intensities_outer[:j], [mean_intensity_outer]])
        ...
        if fast_lookup:
            ...
            band_index_2 = tf.reduce_all(
                [phi_4d <= narrow_band_width, phi_4d >= -narrow_band_width], axis=0)
            band_2 = tf.where(band_index_2)
            u_inner = get_intensity(image, tf.cast((([phi_4d <= 0])), dtype='float32')[
                                    0], filter_patch_size=f_size)
            u_outer = get_intensity(image, tf.cast((([phi_4d > 0])), dtype='float32')[
                                    0], filter_patch_size=f_size)
        ...
        term1 = tf.multiply(tf.cast(lambda1, dtype='float32'), tf.square(
            tf.gather_nd(img, [band]) - mean_intensities_inner))
        term2 = tf.multiply(tf.cast(lambda2, dtype='float32'), tf.square(
            tf.gather_nd(img, [band]) - mean_intensities_outer))
        force = -nu + term1 - term2
        force /= (tf.reduce_max(tf.abs(force)))
        d_phi_dt = tf.cast(force, dtype="float32") + \
            tf.cast(mu * kappa, dtype="float32")
        dt = .45 / (tf.reduce_max(tf.abs(d_phi_dt)) + 2.220446049250313e-16)
        d_phi = dt * d_phi_dt
        update_narrow_band = d_phi
        phi_level = phi_level + tf.scatter_nd([band], tf.cast(
            update_narrow_band, dtype='float32'), shape=[input_image_size, input_image_size])
    ...
    phi = init_phi
    _, phi = tf.while_loop(lambda i, phi: i < iter_limit,
                           _body, loop_vars=[i, phi])
...
adaptive_lr = tf.train.exponential_decay(
    args.lr, global_step, 100000, 0.96, staircase=False, name=None)
    ...
    if is_training:
        if restore:
            ...
            summary_writer_train = tf.summary.FileWriter(
                args.logdir + '/train', graph=sess.graph)
            summary_writer_valid = tf.summary.FileWriter(
                args.logdir + '/valid', graph=sess.graph)
        else:
        ...
        data_provider_train = DataGen.ImageGen(
            train_filename, shuffle_data=True, n_class=1)
        data_provider_valid = DataGen.ImageGen(
            valid_filename, shuffle_data=True, n_class=1)
        try:
            for iters in range(args.train_iter):
                ...
                if gb_step % args.train_sum_freq == 0:
                    summary_str_train = sess.run(
                        train_summary_op, {x: images, y: labels, phase: True})
                    summary_writer_train.add_summary(
                        summary_str_train, gb_step)
                    images, labels, _ = data_provider_valid(args.batch_size)
                    summary_str_valid = sess.run(
                        train_summary_op, {x: images, y: labels, phase: False})
                    summary_writer_valid.add_summary(
                        summary_str_valid, gb_step)
                else:
                    _, Dicevalue = sess.run(
                        [train_op, Dice], {x: images, y: labels, phase: True})
                ...
                if gb_step % args.save_freq == 0 and gb_step != 0:
                    ...
                    saver.save(sess, ckpt_path + '/_globalstep_%02d' %
                               (gb_step), global_step=gb_step)
                    print('model saved for global step %02d' % (gb_step))

        except KeyboardInterrupt:
            print("Manual interruption occurred.")
            saver.save(sess, args.logdir + '/model_globalstep_%02d' %
                       (gb_step), global_step=gb_step)
            print('model saved for global step %02d' % (gb_step))
    else:
        print("########### Inference ############")
        ...
        test_images = np.float64(np.array(h5.get('X')[:, :, :, 0]))/256.0
        test_labels = np.float64(np.array(h5.get('Y')))/256.0

        test_images = np.expand_dims(test_images, axis=3)
        test_labels = np.expand_dims(test_labels, axis=3)

        out_labels = np.zeros(test_labels.shape[:3])

        data_provider_test = DataGen.ImageGen(
            test_filename, shuffle_data=False, n_class=1)
        '''x = tf.placeholder(
            shape=input_shape, dtype=tf.float32, name="x")
        y = tf.placeholder(dtype=tf.float32, name="y")'''

        st_pt = 0
        num_test_imgs = test_images.shape[0]

        while st_pt < num_test_imgs:

            print('Processing Case {} '.format(count+1))
            count += 1

            if st_pt + args.batch_size <= num_test_imgs:
                images = test_images[st_pt:st_pt+args.batch_size]
                labels = test_labels[st_pt:st_pt+args.batch_size]
            ...
            images1, labels1, _ = data_provider_test(args.batch_size)
            ...
            seg_out_acm = sess.run(
                phi_out, {x: images1, y: labels1, phase: False})
            #seg_out_acm = seg_out_acm

            if st_pt + args.batch_size <= num_test_imgs:

                out_labels[st_pt:st_pt+args.batch_size] = seg_out_acm
            else:
